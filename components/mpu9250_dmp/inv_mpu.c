 ...
 * min(int a, int b)
 */
#define MPU9250 // TODO From MakeMenuConfig

#include "i2c/i2c.h"
#include "utils/utils.h"
#include "esp_log.h"
#define i2c_write(a, b, c, d) i2c_write_bytes(a, b, c, d)
#define i2c_read(a, b, c, d)  i2c_read_bytes(a, b, c, d)
#define log_i(msg, ...) ESP_LOGI("inv_mpu", msg, ##__VA_ARGS__);
#define log_e(msg, ...) ESP_LOGE("inv_mpu", msg, ##__VA_ARGS__);
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
    ...
    if (timestamp)
        get_ms(timestamp);
    data[0] = raw;//(long)((35 + ((raw - (float)st.hw->temp_offset) / st.hw->temp_sens)) * 65536L);
    return 0;
}
